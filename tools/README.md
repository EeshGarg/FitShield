...
npm run validate        # node tools/validate-all.js — all 18 audits
```
...
## Validators (all 18 run by `npm run validate`)
...
| `npm run validate:extension` | `extension-audit.js` | the staged package is closed — every manifest, page, and runtime reference resolves — and its API surface stays inside the three declared permissions: the shipped sources are grepped for `chrome.gcm`, `chrome.identity.getProfileUserInfo`, `chrome.system` and six more, each of which would either exceed `storage`/`declarativeNetRequest`/`alarms` or break the no-network, no-telemetry promise outright |
...
tools/
  ...
  validate-all.js               # runs the 18 audits above
  ...
  capture-surfaces.js      make-android-icon.js
  ...
  build-android.js      verify-unpacked.js
